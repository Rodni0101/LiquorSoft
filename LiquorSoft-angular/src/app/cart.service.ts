...
export interface CartItem { id?: number; name: string; price: number; icon: string; quantity: number; }
...
export class CartService {
  ...
  add(product: Omit<CartItem, 'quantity'>): void {
    ...
    if (current) current.quantity += 1; else items.push({ ...product, quantity: 1 });
